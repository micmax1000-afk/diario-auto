...
export function notifyDueReminders(items: NotifiableReminder[]): void {
  ...
  for (const item of toNotify) {
    const statusLabel = item.status === "overdue" ? "Scaduta" : "In arrivo";
    const dateLabel = item.dueDate ? new Date(item.dueDate).toLocaleDateString("it-IT") : "";
    new Notification(`${statusLabel}: ${item.label}`, {
      body: `${item.vehicleName}${dateLabel ? ` · scadenza ${dateLabel}` : ""}`,
