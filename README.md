...
1. Crea su GitHub un repository pubblico chiamato **diario-auto** (senza inizializzarlo con README)
2. Da terminale, dentro questa cartella:
   ...
   ```
3. Su GitHub: **Settings → Pages** → sotto "Build and deployment", imposta Source su **GitHub Actions**
4. Il workflow in `.github/workflows/deploy.yml` pubblica automaticamente ad ogni push su `main`
5. Dopo un paio di minuti l'app sarà su `https://TUO-USERNAME.github.io/diario-auto/`

Nota: se dai al repository un nome diverso da `diario-auto`, aggiorna anche:
- `base: '/diario-auto/'` in `vite.config.ts`
- `start_url` e `scope` in `public/manifest.json`
- `const BASE` in `public/sw.js`
